...
const Form = ({ selectedMember, clearSelection }: Formprops) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (data: Member) => {
    try {
      if (data.id) {
        ...
        if (updateData.data) {
          onUpdate(updateData.data);
          alert(`${data.name} 업데이트 완료`);
        }
      } else {
        const created = await createMember(data);
        if (created.data) {
          onCreate(created.data);
          alert(`${data.name}님 등록 완료`);
        }
      }
      reset();
      ...
      alert(`${userName}님 등록 실패:${(err as Error).message}`);
    }
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          className="w-full py-3 mt-4 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 active:scale-95 transition-transform"
          type="submit"
        >
          제출
        </button>
